...
if (args["SAMPLE_TABLE"] == "default"){
  ...
  sampleTable <- DE.design.full[as.character(DE.design.full$Response)=="Y",]
  rm(DE.design.full)
...
sampleTable[,1]<- factor (as.character(sampleTable[,1]))
model.matrix <- createModelMatrix (formula.text=DE_MODEL, design.matrix=sampleTable[,c(1:2)])
...
dd <- runDESeq (txi=txi.rsem, model.matrix=model.matrix, design.matrix=sampleTable[,c(1:2)], formula.text="~Treatment")
