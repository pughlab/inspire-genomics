...
model.matrix <- createModelMatrix (formula.text=DE_MODEL, design.matrix=sampleTable)
...
dd <- runDESeq (txi=txi.rsem, model.matrix=model.matrix, design.matrix=sampleTable[,c(1:3,4)], formula.text="~Response")
res <- results(dd, contrast = list("ResponseY.Treatment1","ResponseN.Treatment1"))
